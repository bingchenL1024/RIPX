...
x1= x0+xw+dx_sub; 
x2 = x1+xw+dx_sub;
pos = [x0 y0 xw yw; x1 y0 xw yw;x2 y0 xw yw];
...
xlabel('$\Delta t (s)$','Interpreter','latex')
xtickangle(35)
leg = legend([h1 h2 h3],{'$-0.75 \, L_{\mathrm{sz}}$','$-0.5 \, L_{\mathrm{sz}}$','$-0.33\,L_{\mathrm{sz}}$'},'FontSize',leg_fontsz,'Interpreter','latex');
leg.Position = [0.49,0.78,0.1,0.05];
leg.ItemTokenSize(1) = 12;
set(gca,'XTick',0:1:7)
set(gca,'YTick',[-0.1,0:0.2:1])
ax=gca;
set(ax.YLabel,'Position',[-1.65,ax.YLabel.Position(2),0])
set(ax.XLabel,'Position',[ax.XLabel.Position(1),-0.3,0])
xtickangle(0)
grid on
hold off 
niceplot_nobold_nomintick(fig_fontsize)
text(5.82,0.95,subfiglabel{2},'FontSize',subfiglabel_fontsz)
...
for i = 1:24
    for loc =1:3
    plot(fitpara_3loc.slp2(i).t_nond_diva{loc},fitpara_3loc.slp2(i).cxt_data{loc},'Color',[.5 .5 .5],'LineWidth',linewidth-0.1)
    hold on 
    end 
    for loc =1:3
    plot(fitpara_3loc.slp3(i).t_nond_diva{loc},fitpara_3loc.slp3(i).cxt_data{loc},'Color',[.5 .5 .5],'LineWidth',linewidth-0.1)
    hold on 
    end 
    for loc =1:3
    plot(fitpara_3loc.slp4(i).t_nond_diva{loc},fitpara_3loc.slp4(i).cxt_data{loc},'Color',[.5 .5 .5],'LineWidth',linewidth-0.1)
    hold on 
    end 
...
xtickangle(0)
niceplot_nobold_nomintick(fig_fontsize)
text(10.0,0.95,subfiglabel{3},'FontSize',subfiglabel_fontsz)
...
print -dpdf '/data1/bliu/vortpaper/fig_cxtandfit.pdf'
