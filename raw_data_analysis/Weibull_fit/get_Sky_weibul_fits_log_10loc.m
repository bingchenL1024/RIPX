...
% This script does the Sky truncating, interpolating, and Weibul fitting for 10 locations in surfzone
...
load /data1/bliu/data/qa_qc_RIPX_NK.mat % now S(1--120) struct is in memory

% this returns the RIPX run # for the requested parameters.
[p2,v2] = get_sampled_ripx_spread_index(0.1, 2, 0.01, 0.5, 0.02); % p2: 24*1 ; v2: 24*4 [Hs3 fp3 slp spread3]
[p3,v3] = get_sampled_ripx_spread_index(0.1, 2, 0.01, 0.5, 0.03);
[p4,v4] = get_sampled_ripx_spread_index(0.1, 2, 0.01, 0.5, 0.04);

for N=1:length(p4) % 1:24
...
% Get the indexes for a 10 different locations within surfzone 
[iss2] = get_10locs(SS2);
[iss3] = get_10locs(SS3);
[iss4] = get_10locs(SS4);
...
kyy = P2.fck/(2*pi); % so that kyy is in 1/m
xlim2 = [0.0008 0.2];
...
save('/data1/bliu/data/WF_1p2_10loc','lskil2','lskil3','lskil4','wskil2','wskil3','wskil4','Skym2','Skym3','Skym4','kym2','kym3','kym4','wSkym2','wSkym3','wSkym4','wkym2','wkym3','wkym4','wfit2','wfit3','wfit4','iS2tot','iS3tot','iS4tot','iS2s','iS3s','iS4s')