...
% option to plot cxt with bad fit 
...
for runind = 1:24
    ...
    if ~isempty(badfit_temp)
        for i = 1:length(badfit_temp)
            ...
            badfit.slp2.(['runind_',num2str(indbath.slp2(runind))]).t{i}= A.x_itp_fit{badfit_temp(i)};
...
%             fitpara.slp2(runind).c(badfit_temp(i)) = NaN;
    ...
    if contains(plot_badfit,'Yes')
        for i = 1:length(badfit_temp)
            ...
            t_badfit = A.x_itp_fit{badfit_temp(i)};
...
for runind = 1:24
    ...
    if ~isempty(badfit_temp)
        for i = 1:length(badfit_temp)
            ...
            badfit.slp3.(['runind_',num2str(indbath.slp3(runind))]).t{i}= A.x_itp_fit{badfit_temp(i)};
...
for runind = 1:24
    ...
    if ~isempty(badfit_temp)
        for i = 1:length(badfit_temp)
            ...
            badfit.slp4.(['runind_',num2str(indbath.slp4(runind))]).t{i}= A.x_itp_fit{badfit_temp(i)};
...
save('/data1/bliu/data/cxt_alongct_x_fitpara_qced.mat','fitpara')

%save('/data1/bliu/data/cxt_alongct_x_maxvar_fitpara_qced.mat','fitpara')
