...
ind_good_All{runnum,1} =ind_good; 
...
for i=1:length(ind_good) %cross shore dim
    ...
    c_coef_all = 0.5+((1:c_coef_max)-1).*0.1;
    for c_coef_ind = 1:c_coef_max %search for different c 
...
cxt_alongct_x_ALL{runnum,1} = cxt_alongct;
x_nond_All{runnum,1} =  x_nond(ind_good); 
t_itp_All{runnum,1} = t_all;
c_coef_maxvar_All{runnum,1} = c_coef_maxvar;
c_maxvar_All{runnum,1} = c_maxvar_all;
...
save('/data1/bliu/data/cxt_alongct_x_maxvar','cxt_alongct_x_ALL','t_itp_All','c_coef_maxvar_All','c_maxvar_All','x_itp','x_nond_All','ind_good_All')
