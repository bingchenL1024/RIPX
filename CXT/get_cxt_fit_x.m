...
% It also included parameters for nondim parameter calculation (h,Tp,H,...)
...
opts.Display = 'Off';
opts.Lower = [0 0 -pi];
opts.StartPoint = [1 1 0.4 ]; % beginning parameters - amp, mu, std.
...
for ind_slp2 = 1:24
    ...
    cxt_alongct_1run  = cxt_alongct_x_ALL{runnum};
    ...
    for i = 1:5:dim_cxt(2) %crossshore scale ----------fit every 5 meter
        ...
        [f,gof]=fit(x_itp_fit(include)',cxt_data_fit(include),ft, opts);
        cxt_fit = exp(-x_itp_fit(include)./f.a).*cos(x_itp_fit(include)/(f.b)+f.c)/(cos(f.c));
%%%%%%%%%%%%%% Quick QC
% figure(1);plot(x_itp_fit(include)',cxt_data_fit(include)); hold on;plot(x_itp_fit(include)',cxt_fit);hold off;legend('data','fit')
% pause(0.2)
        ...
        fitpara.slp2{ind_slp2,1}.x_itp_fit{j,1} = x_itp_fit(include)';        
...
for ind_slp3 = 1:24
    ...
    for i = 1:5:dim_cxt(2) %fit every 5 meter
        ...
        include = ~isnan(cxt_data_fit);
%         figure()
%         plot(x_itp_fit(include)',cxt_fit)
%         hold on 
%         plot(x_itp_fit(include)',cxt_data_fit(include))
%         hold off 
        [f,gof]=fit(x_itp_fit(include)',cxt_data_fit(include),ft, opts);
        cxt_fit = exp(-x_itp_fit(include)./f.a).*cos(x_itp_fit(include)/(f.b)+f.c)/(cos(f.c));
%figure(1);plot(x_itp_fit(include)',cxt_data_fit(include)); hold on;plot(x_itp_fit(include)',cxt_fit);hold off;legend('data','fit')
% pause(0.2)
        ...
        fitpara.slp3{ind_slp3,1}.x_itp_fit{j,1} = x_itp_fit(include)';        
...
% pause(0.2)
        ...
        fitpara.slp4{ind_slp4,1}.x_itp_fit{j,1} = x_itp_fit(include)';        
...
readme = 'created in get_cxt_fit_x (fit cxt(dx))';
save('/data1/bliu/data/cxt_alongct_x_fitpara','fitpara','readme')
%save('/data1/bliu/data/cxt_alongct_x_maxvar_fitpara','fitpara','readme')