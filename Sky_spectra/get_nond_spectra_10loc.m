...
for N = 1:24
    for xx=1:length(Skym2(1,:))
        ...
        % Sky3_nond_kym_S0(N,xx,:) = Sky3_cutoff_runmean(N,xx,:).*kym3(N,xx)./wfit3.So(N,xx); 
        % Sky4_nond_kym_S0(N,xx,:) = Sky4_cutoff_runmean(N,xx,:).*kym4(N,xx)./wfit4.So(N,xx);
        ...
        % ky2_nond(N,xx,:) = ky2_wb(N,xx,:)/wkym2(N,xx);
        % ky3_nond(N,xx,:) = ky3_wb(N,xx,:)/wkym3(N,xx);
        % ky4_nond(N,xx,:) = ky4_wb(N,xx,:)/wkym4(N,xx);
